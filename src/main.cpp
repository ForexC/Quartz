...
static const int64_t nTargetTimespan = 60 * 5;  // 5m
unsigned int nTargetSpacing = 1 * 60; // Quartz - 1 minute blocks
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 60;
unsigned int nStakeMaxAge = -1;
unsigned int nModifierInterval = 10 * 60; // Quartz - time to elapse before new modifier is computed
...
const string strMessageMagic = "Quartz Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Quartz", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "22 Apr 2015";
        CTransaction txNew;
        txNew.nTime = 1429732858;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1429732858;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 22520133;
        uint256 hash = block.GetHash();
        printf("%s\n", hash.ToString().c_str());
        printf("%s\n", hashGenesisBlock.ToString().c_str());
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x066ddf30624b301561e1819fab39e1740282a601391df7f56244f130df309223"));
...
unsigned char pchMessageStart[4] = { 0xaa, 0x22, 0xff, 0xaa };
